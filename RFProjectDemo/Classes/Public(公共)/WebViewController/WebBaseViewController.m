...
@property (nonatomic, assign) UIWebViewRefreshType refreshType;     /** 是否返回刷新 */
@property (nonatomic, assign) CYwebAddAppUINameType addUINameType;  /** 添加右按钮的类型 */
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([LoginManager isLogin] && self.loginInRefresh) {
...
- (WKWebView *)webView {
    if (!_webView) {
        ...
        _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, k_Navigation_Bar_Height, KW, KH- k_Navigation_Bar_Height) configuration:confi];
...
//    if (self.addUINameType == CYwebAddAppUINameTypeShare) {
//        //  分享
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([requestString hasPrefix:@"itms-services://"]) {
        ...
        NSArray *urls = [requestString componentsSeparatedByString:@"?"];
...
- (NSString *)H5RequestUrlParamas:(NSURL *)requestUrl {
    NSDictionary *headerDict = [AppManager baseDictionary] ;
...
- (BOOL)tellPhoneInterupt:(NSString *)urlStr {
    if ([urlStr hasPrefix:@"sms:"] || [urlStr hasPrefix:@"tel:"]) {
        ...
        [AlertSheetActionManager sheetActionTitle:message message:nil arrTitleAction:arr superVc:self blockClick:^(NSInteger index) {
            if (0 == index) {
...
- (void)openNativeShareWithParams:(NSString *)paramsString {
    ...
    if ([appLogin integerValue]==1 && ![LoginManager isLogin]) {
        [LoginManager presentLoginVCWithController:self];
    } else {
        //[XLUmengShareHelper startShareWithDic:resultDict];
    }
