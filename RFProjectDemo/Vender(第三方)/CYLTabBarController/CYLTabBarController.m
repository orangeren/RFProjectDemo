...
//        [center addObserver:self selector:@selector(tokenInvalid:) name:NotTokenInvalidNotification object:nil];
//        //  Token 值失效 弹出登录框
//        [center addObserver:self selector:@selector(tokenInvalidPushLogin:) name:NotTokenInvalidLoginPushNotification object:nil];
    }
    return self;
}
...
////退出登录
//-(void)tokenInvalid:(NSNotification *)not{
//    [SVProgressHUD dismiss];
//    UIViewController *controller = [UIViewController currentViewController];
//    NSString *currentControllerString = NSStringFromClass([controller class]);
//    if ([currentControllerString isEqualToString:@"WJYViewController"]) {
//
//    } else {
//        [WJYAlertView showOneButtonWithTitle:@"温馨提示" Message:@"您的账号在其他设备登录，如果这不是您的操作，请及时修改您的登录密码。" ButtonType:WJYAlertViewButtonTypeWarn ButtonTitle:@"知道了" Click:^{
//            NSArray * arr = self.viewControllers;
//            UINavigationController * navVc = arr[self.selectedIndex];
//            [navVc popToRootViewControllerAnimated:YES];
//            [LoginManager presentLoginVCWithController:arr.firstObject];
//        }];
//    }
//}
//// token失效弹出 登录页面
//-(void)tokenInvalidPushLogin:(NSNotification *)not{
//    [SVProgressHUD dismiss];
//    // 弹出登录
//    UIViewController *controller = [UIViewController currentViewController];
//    NSString *currentControllerString = NSStringFromClass([controller class]);
